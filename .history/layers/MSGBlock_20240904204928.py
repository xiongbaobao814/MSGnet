...
class Attention_Block(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        y = x = self.norm1(x)
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))   # new_x:[32, 2048,96, 512]
...
class GraphBlock(nn.Module):
    def __init__(self, c_out, d_model, conv_channel, skip_channel, gcn_depth, dropout, propalpha, seq_len, node_dim):
        ...
        self.nodevec1 = nn.Parameter(torch.randn(c_out, node_dim), requires_grad=True)  # 定义两个可学习的参数矩阵
        self.nodevec2 = nn.Parameter(torch.randn(node_dim, c_out), requires_grad=True)  # [10,7],[7,10]
    ...
    def forward(self, x):
        adp = F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim=1)  # 计算自适应邻接矩阵[7,7]
        out = x.unsqueeze(1).transpose(2, 3)    # [32, 1, 96, 512]
