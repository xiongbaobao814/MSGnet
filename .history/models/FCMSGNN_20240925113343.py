...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        A_input_ = self.nonlin_map2(A_input_)                                # [1800, 32]       # [224, 32]
        A_input_ = torch.reshape(A_input_, [bs, scale_num, num_nodes, -1])   # [100, 2, 9, 32]  # [32, 7, 32]

        ## positional encoding
        X_ = torch.reshape(A_input_, [bs, scale_num, num_nodes, -1])         # [100, 2, 9, 32]  # [32, 7, 32]
        # X_ = torch.transpose(X_, 1, 2)
        # X_ = torch.reshape(X_, [bs*num_nodes, scale_num, -1])              # [900, 2, 32]         
        X_ = self.positional_encoding(X_)
        X_ = torch.reshape(X_, [bs, num_nodes, scale_num, -1])                                  # [32, 7, 32]
        # X_ = torch.transpose(X_, 1, 2)
        # A_input_ = X_                                               # [100, 2, 9, 32]      # [32, 7, 32]
