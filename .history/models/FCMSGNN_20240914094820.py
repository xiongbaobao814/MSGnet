...
class Model(nn.Module):
    ...
    # def forward(self, X):
        ...
        ## Graph Convolution
        scale_list, scale_weight = FFT_for_Period(A_input_, self.k)
        scale = scale_list[0]
        # padding
        if (self.seq_len) % scale != 0:
            length = (((self.seq_len) // scale) + 1) * scale
            padding = torch.zeros([A_input_.shape[0], (length - (self.seq_len)), A_input_.shape[2]]).to(A_input_.device)
            out = torch.cat([A_input_, padding], dim=1)
        else:
            length = self.seq_len
            out = A_input_     # [32, 96, 512]
        out = out.reshape(bs, length // scale, scale, dimension) # [32, 1, 96, 512]
        
        
        scale_input = self.scalegraph(A_input_)
        MPNN_output1 = self.MPNN1(A_input_)                     # [100, 1, 9, 16]
        MPNN_output2 = self.MPNN2(A_input_)                     # [100, 1, 9, 16]
