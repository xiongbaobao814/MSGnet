...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.num_node事 = configs.num_node
        ...
        # 图构建和聚合：图卷积池化MPNN模块
        self.MPNN1 = GraphConvpoolMPNN_block_v6(2*self.hidden_dim, self.hidden_dim, self.num_node, self.time_length, 
                                                time_window_size=self.moving_window[0], stride=self.stride[0], 
                                                decay=self.decay, pool_choice=self.pooling_choice)
        self.MPNN2 = GraphConvpoolMPNN_block_v6(2*self.hidden_dim, self.hidden_dim, self.num_node, self.time_length, 
                                                time_window_size=self.moving_window[1], stride=self.stride[1], 
                                                decay=self.decay, pool_choice=self.pooling_choice)       
        # FC Graph Convolution
        self.fc = nn.Sequential(OrderedDict([
            ('fc1', nn.Linear(self.hidden_dim * self.num_windows * self.num_node, 2*self.hidden_dim)),
    ...
    # def forward(self, X):
        bs, dimension, num_node = x_enc.size()                        # 100, 2, 9, 64

        ### Graph Generation
        A_input = torch.reshape(x_enc, [bs*num_node, dimension, 1])    # [1800, 64, 1]
        A_input_ = self.nonlin_map(A_input)                             # [1800, 18, 10]
        A_input_ = torch.reshape(A_input_, [bs*num_node, -1])       # [1800, 180]
        A_input_ = self.nonlin_map2(A_input_)                           # [1800, 32]
        A_input_ = torch.reshape(A_input_, [bs, num_node, -1])      # [100, 2, 9, 32]

        ## positional encoding before mapping starting
        X_ = torch.reshape(A_input_, [bs, num_node, -1])          # [100, 2, 9, 32]
        X_ = torch.transpose(X_, 1, 2)
        X_ = torch.reshape(X_, [bs*num_node, -1])                 # [900, 2, 32]
        X_ = self.positional_encoding(X_)
        X_ = torch.reshape(X_, [bs, num_node, -1])
        X_ = torch.transpose(X_, 1, 2)
