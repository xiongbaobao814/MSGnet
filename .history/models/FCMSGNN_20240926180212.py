...
def nonlin_map2():
    nn.Sequential(nn.Linear(self.lstmout_dim*self.conv_out, scale), nn.BatchNorm1d(scale))   # 180 → 32, 144->32
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.nonlin_map2 = nn.Sequential(nn.Linear(self.lstmout_dim*self.conv_out, scale),
                                        nn.BatchNorm1d(scale))   # 180 → 32, 144->32
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        A_input = A_input.reshape(bs, scale_num, scale, num_nodes)           # [32, 2, 48, 7]   
        ...
        A_input_ = self.nonlin_map2(A_input_, scale)                                # [1800, 32]      [224, 32]     [448,32]
        ...
        features = torch.reshape(features, [bs, scale_num, num_nodes, -1]) # [100, 2, 9, 32]  [32, 1, 7, 32]  [32, 2, 7, 32]
