...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if (self.seq_len) % scale != 0:
            ...
            length = self.seq_len
            A_input_ = x_enc     # [32, 96, 512]
        A_input_ = A_input_.reshape(bs, length // scale, scale, num_nodes)      # [32, 1, 96, 512]   
        
        ## Graph Generation
        A_input = torch.reshape(A_input_, [bs*num_nodes, dimension, 1])    # [1800, 64, 1]     # [224, 96, 1]
